...
function MenuEditor(idSelector, options) {
    ...
    function createModule(arrayItem, depth) {
        var level = (typeof (depth) === 'undefined') ? 0 : depth;
        ...
        $.each(arrayItem, function (k, v) {
            ...
            var itemObject = {title: "", postid: "", limit: "", type: ""};
            var temp = $.extend({}, v);
            ...
            var $i = $('<i>').text(v.type);
            var $span = $('<span>').addClass('title').append(v.title).css('margin-right', '5px');
            var $divbtn =  tButtonModuleGroup();
            $div.append($i).append("&nbsp;").append($span).append($divbtn);
            $li.append($div);
            if (isParent) {
                $li.append(createModule(v.children, level + 1));
            }
            $elem.append($li);
        });
        return $elem;
    ...
    this.moduleAdd = function (strJson) {
        var arrayItem = (Array.isArray(strJson)) ? strJson : stringToArray(strJson);
        if (arrayItem !== null) {
            var menu = createModule(arrayItem);
            MenuEditor.updateButtons($main);
        }
    };
};
