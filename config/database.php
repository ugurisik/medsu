...
class MysqliDb {
    ...
    {
        $groupByField = preg_replace("/[^-a-zA-Z0-9\.\(\),_\* <>=!]+/i", '', $groupByField);
    ...
    function yetkikont($type,$userID){
        $this->where ("userID", $userID);
        $this->where ("type", $type);
        $userMeta = $this->getOne ("users_meta");
        return $userMeta['type_meta'];
    }
    function translate($string){
        $this->where ("langID",LANGID);
        $this->where ("type",$string);
        $r = $this->getOne("langs_meta");
        return $r['type_meta'];
    }
    function getSetMeta($setType,$type){
        $this->where ("setType",$setType);
        $this->where ("type",$type);
        $r = $this->getOne("settings_meta");
        return $r['type_meta'];
    }
    function getPostMeta($postID,$type){
        $this->where ("postID",$postID);
        $this->where ("type",$type);
        $r = $this->getOne("post_meta");
        return $r['type_meta'];
    }
    function emptyPost($postData){
